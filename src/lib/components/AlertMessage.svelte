...
{#if !dismissed}
	<div class="flex items-start gap-3 p-4 border rounded-lg {styles[type]}" role="alert">
		<div class="mt-0.5 shrink-0 {iconColors[type]}">
			{#if type === 'info'}
		...
		{#if dismissible}
			<button onclick={dismiss} class="shrink-0 p-1 rounded hover:bg-black/5 dark:hover:bg-white/5" aria-label="Dismiss alert">
